document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateTable(reportsToShow) { /* ... unchanged with its previous console.logs ... */ }
    function updateKPIs(currentBaseFilteredData, currentDateRangeFilteredData) { /* ... unchanged with its previous console.logs ... */ }
    ...
    function populateFilter(data) {
        ...
        departmentFilter.innerHTML = '<option value="all">عرض الكل</option>'; // Reset options
        ...
        console.log("populateFilter: Filter populated with", departments.length, "departments.");
    }

    function renderCurrentPage() {
        console.log("--- renderCurrentPage START --- Current Page:", currentPage);
        if (!allReportsData || !Array.isArray(allReportsData)) { // Check if allReportsData is not an array
            console.error("renderCurrentPage: allReportsData is not a valid array or not loaded. Aborting render.");
        ...
        if (allReportsData.length === 0 && tableBody) { // If it's an empty array specifically after fetch
             console.warn("renderCurrentPage: allReportsData is an empty array.");
             // Message is already handled by fetchData in this case.
        }
        ...
        console.log("--- renderCurrentPage END --- Reports shown:", reportsToShow.length);
    }
    ...
    function toggleTheme() {
        ...
        // Re-render charts and calendar if they exist and are visible, as theme variables change
        ...
        if (activeViewId === 'analytics' && chartsDrawn) {
            ...
            // Data for charts should be based on current filters, not necessarily allReportsData
        ...
        if (activeViewId === 'calendar' && calendarInitialized) {
            ...
            initializeCalendar(); // This will destroy and re-create with new theme colors
        }
    }
    ...
    function loadTheme() {
        ...
        if (savedTheme === 'dark') {
            ...
            document.body.classList.remove('dark-mode'); // Ensure light is default
    ...
    function initializeCalendar() {
        ...
        const searchTerm = searchInput ? searchInput.value.toLowerCase() : "";
        const selectedDept = departmentFilter ? departmentFilter.value : "all";
        // Calendar data should reflect current search & department filters
        ...
        const calendarEvents = calendarData.map(report => {
            if (!report || !Array.isArray(report) || report.length < 5) {
                ...
                return null; // Skip invalid report
            }
        ...
        }).filter(event => event !== null); // Remove any null events from invalid reports

        console.log("initializeCalendar: Sample calendarEvents (first 2):", JSON.stringify(calendarEvents.slice(0,2)));
    ...
    function handleNavigation(event) {
        ...
        if (viewId === 'calendar') {
            ...
            initializeCalendar(); // This will now re-filter data for calendar
        }
    ...
    } else { console.warn("No navLinks found to attach listeners."); }
